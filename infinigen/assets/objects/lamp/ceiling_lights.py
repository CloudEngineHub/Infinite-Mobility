...
class CeilingLightFactory(AssetFactory):
    ...
    def create_placeholder(self, i, **params):
        ...
        butil.modify_mesh(
            obj,
            "NODES",
            node_group=nodegroup_ceiling_light_geometry(**params),
...
@node_utils.to_modifier(
    "nodegroup_ceiling_light_geometry", singleton=False, type="GeometryNodeTree"
)
def nodegroup_ceiling_light_geometry(
    nw: NodeWrangler, preprocess: bool = False, **kwargs
):
    ...
    if "inputs" in kwargs.keys():
        group_input = nw.new_node(
            Nodes.GroupInput,
            expose_input=[
                ("NodeSocketFloatDistance", "Radius", kwargs["inputs"]["Radius"]),
                ("NodeSocketFloat", "Thickness", kwargs["inputs"]["Thickness"]),
                ("NodeSocketFloat", "InnerRadius", kwargs["inputs"]["InnerRadius"]),
                ("NodeSocketFloat", "Height", kwargs["inputs"]["Height"]),
                ("NodeSocketFloat", "InnerHeight", kwargs["inputs"]["InnerHeight"]),
                ("NodeSocketFloat", "Curvature", kwargs["inputs"]["Curvature"]),
    ...
    store_ceiling_light_1 = nw.new_node(
    ...
    extrude_mesh = nw.new_node(
        Nodes.ExtrudeMesh,
        input_kwargs={
            "Mesh": store_ceiling_light_1.outputs["Geometry"],
    ...
    store_ceiling_light_2 = nw.new_node(
        Nodes.StoreNamedAttribute,
        input_kwargs={
    ...
    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={
            "Geometry": [set_shade_smooth, store_ceiling_light_2.outputs["Geometry"]]
        },
    )
    ...
    store_ceiling_light_3 = nw.new_node(
        Nodes.StoreNamedAttribute,
        input_kwargs={
    ...
    transform = nw.new_node(
        Nodes.Transform,
        input_kwargs={
            "Geometry": store_ceiling_light_3.outputs["Geometry"],
    ...
    store_ceiling_light_4 = nw.new_node(
        Nodes.StoreNamedAttribute,
        input_kwargs={
    ...
    join_geometry_2 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={
            "Geometry": [transform, store_ceiling_light_4.outputs["Geometry"]]
        },
    )
        ...
        for i, name in enumerate(names):
            ...
            for j in range(1, parts[i] + 1):
                ...
                separate_geometry = nw.new_node(
                    ...
                    input_kwargs={
                        "Geometry": join_geometry_3.outputs["Geometry"],
