...
from infinigen.assets.utils.object import save_obj_parts_join_objects, join_objects, save_obj_parts_add, join_objects_save_whole, save_obj_parts_join_objects
...
class CupFactory(TablewareFactory):
    ...
    def add_handle(self, obj, handle_location, handle_angle, path=None, i="unknown"):
        ...
        save_obj_parts_add([handle], path, i, name=["handle"], obj_name="Cup", first=True)
        return join_objects([obj, handle])
        return save_obj_parts_join_objects(
            [obj, handle], path, i, name=["obj", "handle"], obj_name="Cup", first=True
        )
