...
class PrickyPearBaseCactusFactory(BaseCactusFactory):
    ...
    def build_leaves(self, level=0, path=None, i="unknown"):
        if level == 0:
        ...
        obj = save_parts_join_objects(
            [base, *leaves], path, i, name=["base"] + ["leaves"] * n
        )
        return obj

    def create_asset(self, face_size=0.01, **params) -> bpy.types.Object:
        obj = self.build_leaves(2, params.get("path", None), params.get("i", "unknown"))
