...
def geometry_nodes_add_cabinet_top(nw: NodeWrangler):
    ...
    store_1 = nw.new_node(
            ...
            input_kwargs={
                "Geometry": generatetabletop,  # Geometry with UV map
                "Name": "top",
...
class KitchenSpaceFactory(AssetFactory):
    ...
    def create_asset(self, **params):
        ...
        cabinet_bottom = cabinet_bottom_factory(i = params.get("i", None), path=params.get("path", None), save_whole=False)
        #save_geometry_new(cabinet_bottom, 'whole', 0, params.get("i", None), params.get("path", None), True, use_bpy=True)
        ...
        for j, name in enumerate(names):
                for k in range(1, parts[j] + 1):
                    res = save_geometry_new(kitchen_space, name, k, params.get("i", None), params.get("path", None), first, use_bpy=True)
                    if res:
                        first = False

        save_geometry_new(kitchen_space, 'whole', 0, params.get("i", None), params.get("path", None), True, use_bpy=True)
