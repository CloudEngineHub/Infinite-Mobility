...
def nodegroup_ramped_edge(nw: NodeWrangler):
    ...
    num = nw.new_node(
        Nodes.Math, input_kwargs={0: group_input.outputs["height"], 1: 3},
        attrs={"operation": "MULTIPLY"},
    )
...
def geometry_door_nodes(nw: NodeWrangler, **kwargs):
    ...
    names = ["ramped_edge", "panel_edge_frame"]
    parts = [6, 4]
    ...
    knob_radius.outputs[0].default_value = kwargs["knob_R"]
    ...
    for h in kwargs["attach_height"]:
        ...
        # Store unique 'cube' for Cube 1
        store_attach = nw.new_node(
            Nodes.StoreNamedAttribute,
            input_kwargs={
                "Geometry": attach,  # Geometry with UV map
                "Name": "attach",
                "Value": h+1,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "POINT", "data_type": "INT"},
        )
        ...
        set_material_1 = nw.new_node(
            Nodes.SetMaterial,
            input_kwargs={"Geometry": store_attach, "Material": get_shelf_material("metal")},
        )
        attach_gadgets.append(set_material_1)

    names.append("attach")
    parts.append(kwargs["attach_height"])
            ...
            if name == "attach":
                ...
                for k in range(1, 4):
                    ...
                    separate_geometry_1 = nw.new_node(
                        node_type=Nodes.SeparateGeometry,
                        input_kwargs={
                            "Geometry": separate_geometry.outputs["Geometry"],
...
def geometry_cabinet_nodes(nw: NodeWrangler, **kwargs):
    ...
    attaches = []
    for i, pos in enumerate(kwargs["attach_pos"]):
        cube = nw.new_node(
        ...
        store_cube = nw.new_node(
            ...
            input_kwargs={
                "Geometry": cube,  # Geometry with UV map
                "Name": "attach_pos",
                "Value": 1 + 2*i,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "POINT", "data_type": "INT"},
        )
        ...
        transform = nw.new_node(
            Nodes.Transform, input_kwargs={"Geometry": store_cube.outputs["Geometry"], "Translation": combine_xyz}
        )
        ...
        store_cube_1 = nw.new_node(
            ...
            input_kwargs={
                "Geometry": cube_1,  # Geometry with UV map
                "Name": "attach_pos",
                "Value": 2 + 2*i,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "POINT", "data_type": "INT"},
        )

        join_geometry = nw.new_node(
            Nodes.JoinGeometry, input_kwargs={"Geometry": [transform, store_cube_1.outputs["Geometry"]]}
        )
...
class CabinetBaseFactory(AssetFactory):
    ...
    def create_asset(self, idx=0, **params):
        ...
        shelf, right_door, left_door = self.get_cabinet_components(i=idx)

        # create cabinet
        cabinet_params = self.get_cabinet_params(i=idx)
        surface.add_geomod(
            obj,
