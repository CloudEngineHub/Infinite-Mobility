...
def nodegroup_array_fill_line(nw: NodeWrangler):
    ...
    for i in range(5):
        compare = nw.new_node(
            Nodes.Compare, input_kwargs={0: index, 1: i}, attrs={"operation": "EQUAL"}
        )
        ...
        store_point = nw.new_node(
            Nodes.StoreNamedAttribute,
            input_kwargs={
                "Geometry": separate_geometry,  # Geometry with UV map
                "Name": "point",
                "Value": i,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "POINT", "data_type": "INT"},
        )
        points.append(store_point)
    ...
    if points:
        ...
        join_geometry = mesh_line

    instance_on_points_1 = nw.new_node(
        Nodes.InstanceOnPoints,
        input_kwargs={"Points": join_geometry, "Instance": group_input.outputs["Instance"]},
    )
    ...
    for i in range(5):
        compare = nw.new_node(
            Nodes.Compare, input_kwargs={0: named_attribute, 1: i}, attrs={"operation": "EQUAL"}
        )
        separate_geometry = nw.new_node(
            Nodes.SeparateGeometry,
            input_kwargs={
        ...
        store_1 = nw.new_node(
            ...
            input_kwargs={
                "Geometry": separate_geometry,  # Geometry with UV map
                "Name": "cushion_short",
                "Value": i + 1 + total_count,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "INSTANCE"},
        )
        realize_instances = nw.new_node(
            Nodes.RealizeInstances, input_kwargs={"Geometry": store_1.outputs["Geometry"]}
        )
        geometries.append(realize_instances)

    if geometries:
        parts = nw.new_node(
            Nodes.JoinGeometry,
            input_kwargs={"Geometry": geometries},
        )
    else:
        parts = instance_on_points_1
    total_count += 5
...
def nodegroup_sofa_geometry(nw: NodeWrangler):
    ...
    join_geometry_2 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [store_arm_2, store]},
    )
    ...
    join_geometry_3 = nw.new_node(
        Nodes.JoinGeometry, input_kwargs={"Geometry": [join_geometry_2, store_board_2]}
    )
    ...
    if points:
        ...
        join_geometry = transform_geometry_10
    ...
    for i in range(10):
        ...
        store_1 = nw.new_node(
            ...
            input_kwargs={
                "Geometry": separate_geometry,  # Geometry with UV map
                "Name": "legs",
                "Value": i + 6,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "INSTANCE"},
        )
        realize_instances = nw.new_node(
    ...
    transform_geometry_11 = nw.new_node(
        Nodes.Transform,
        input_kwargs={"Geometry": parts, "Scale": combine_xyz_19},
    )
    ...
    switch_4 = nw.new_node(
        Nodes.Switch,
        input_kwargs={1: group_input.outputs["Footrest"], 15: transform_geometry_6},
    )
    ...
    join_geometry_10 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [switch_5.outputs[6], parts]},
    )

    join_geometry_base_board = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [base_board, join_geometry_10]},
    )

    store_board_1 = nw.new_node(
        Nodes.StoreNamedAttribute,
        input_kwargs={
            "Geometry": join_geometry_base_board,
            "Name": "base_board",
            "Value": 1,
        }
    )
    ...
    join_geometry_4 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [join_geometry_3, store_board_1]},
    )
    ...
    if points:
        ...
        join_geometry = transform_geometry_2

    instance_on_points = nw.new_node(
        Nodes.InstanceOnPoints,
        input_kwargs={"Points": join_geometry, "Instance": transform_geometry_8},
    )
    ...
    join_geometry_5 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [join_geometry_4, parts]},
    )
    ...
    transform_geometry_7 = nw.new_node(
        Nodes.Transform,
        input_kwargs={"Geometry": store, "Scale": combine_xyz_11},
    )
...
class SofaFactory(AssetFactory):
    ...
    def delete_by_name_attr(self, obj, name, idx):
        bpy.context.view_layer.objects.active = obj
        bpy.ops.object.mode_set(mode='EDIT')
        bm = bmesh.from_edit_mesh(obj.data)
        print(bm.verts.layers.float.keys(), bm.faces.layers.float.keys(), bm.edges.layers.float.keys())
        if name not in bm.verts.layers.float:
            bpy.ops.object.mode_set(mode='OBJECT')
            return 
        if name is not None and name != 'whole':
            attr_layer = bm.verts.layers.float[name]
            found_verts = [v for v in bm.verts if v[attr_layer] == idx]
        else:
            found_verts = [v for v in bm.verts]

        bmesh.ops.delete(bm, geom=found_verts, context='VERTS')
        bpy.ops.object.mode_set(mode='OBJECT')
    
    def calc_all_by_name_attr(self, obj, names, parts):
        bpy.context.view_layer.objects.active = obj
        bpy.ops.object.mode_set(mode='EDIT')
        bm = bmesh.from_edit_mesh(obj.data)
        res = 0
        ...
        for i, name in enumerate(names):
            ...
            if name is not None and name != 'whole':
                attr_layer = bm.verts.layers.float[name]
                for i in range(1, parts[i] + 1):
                    found_verts = [v for v in bm.verts if v[attr_layer] == i]
    ...
    def create_asset(self, i, placeholder, face_size, **params):
        ...
        names = ['arm', 'legs', 'base_board', 'back_board', 'cushion_short']
        parts = [2, 10, 1, 1, 70]

        count = self.calc_all_by_name_attr(hipoly, names, parts)
        print(count)
        if count == 17:
             if uniform() < 0.5:
                 self.delete_by_name_attr(hipoly, 'cushion_short', 21)
             else:
                 self.delete_by_name_attr(hipoly, 'cushion_short', 1)

        first = True
        for j, name in enumerate(names):
            for k in range(1, parts[j] + 1):
                save_geometry_new(hipoly, name, k, i, params.get("path", None), first)
                first = False

        save_geometry_new(hipoly, 'whole', 0, i, params.get("path", None), True)
