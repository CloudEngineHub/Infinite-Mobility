...
class ChairFactory(AssetFactory):
    ...
    def create_asset(self, **params) -> bpy.types.Object:
        ...
        leg_decors = self.make_leg_decors(legs)
        parts.extend(leg_decors)

        arms = []
        if self.has_arm:
            arms = self.make_arms(seat, backs)
            parts.extend(arms)
        back_decors = self.make_back_decors(backs)
        parts.extend(back_decors)
    ...
    def make_leg_decors(self, legs, **params):
        decors = []
    ...
    def make_back_decors(
        self, backs, finalize=True, path="back_decors", i="unknown", name=None
    ):
        obj = join_objects([deep_clone_obj(b) for b in backs])
        # obj = save_obj_parts_join_objects([deep_clone_obj(b) for b in backs], path, i, name=["backs"] * len(backs))
        x, y, z = read_co(obj).T
        ...
        #     bpy.ops.mesh.delete()
        for z_min, z_max in self.back_profile:
            obj_clone = deep_clone_obj(obj)
            center = read_edge_center(obj)
            with butil.ViewportMode(obj_clone, "EDIT"):
                bpy.ops.mesh.select_mode(type="EDGE")
                select_edges(
                    obj_clone,
                    (z_min * self.back_height <= center[:, -1])
                ...
                back = bpy.context.selected_objects
                backs_1.extend(back)
                butil.select_none()
        ...
        for back in backs_1:
            ...
            if finalize:
                butil.modify_mesh(back, "BEVEL", width=self.bevel_width, segments=8)
        # butil.modify_mesh(
        #     obj,
        #     "SOLIDIFY",
        #     thickness=np.minimum(self.thickness, self.back_thickness),
        ...
        if self.back_type == "vertical-bar":
            ...
            # other = save_obj_parts_join_objects([deep_clone_obj(b) for b in backs], path, i, name, name=["backs"] * len(backs))
