...
class BedFrameFactory(ChairFactory):
    ...
    def make_leg_decors(self, legs, path="outputs/leg_decors", i="unknown"):
        if self.leg_decor_type == "none":
            return super().make_leg_decors(legs, path, i)
        obj = join_objects([deep_clone_obj(_) for _ in legs])
        save_obj_parts_add(
            [deep_clone_obj(_) for _ in legs], path, i, name=["leg_decors"] * len(legs)
        )
        x, y, z = read_co(obj).T
    ...
    def make_back_decors(self, backs, finalize=True, path="back_decors", i="unknown"):
        decors = super().make_back_decors(backs, path=path, i=i, name="back_decors")
        # save_objects_obj(decors, path, i, name=["back_decors"] * len(decors))
        match self.back_type:
            case "coiled":
                obj = self.make_back(backs, path, i)
                self.divide(obj, self.dot_distance)
                make_coiled(obj, self.dot_distance, self.dot_depth, self.dot_size)
                obj.scale = (1 - 1e-3,) * 3
                write_attribute(obj, 1, "panel", "FACE")
    ...
    def make_back(self, backs, path=None, i="unknown"):
        obj = join_objects([deep_clone_obj(b) for b in backs])
        save_obj_parts_add(
            [deep_clone_obj(b) for b in backs], path, i, name=["back"] * len(backs)
        )
        with butil.ViewportMode(obj, "EDIT"):
            bpy.ops.mesh.select_all(action="SELECT")
            bpy.ops.mesh.convex_hull()
