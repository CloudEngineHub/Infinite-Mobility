...
class StraightStaircaseFactory(AssetFactory):
    ...
    def make_posts(self, locs, widths, path, idx):
        names = []
        parts = []
        ...
        for loc, width in zip(locs, widths):
            ...
            for i, l in enumerate(loc):
                ...
                obj = mesh2obj(data2mesh(loc[i].reshape(1, -1)))
                with butil.ViewportMode(obj, "EDIT"):
                    bpy.ops.mesh.select_all(action="SELECT")
                ...
                parts.append(obj)
                names.append("post")
        # save_objects_obj(parts, path, idx, names)
        return parts
    ...
    def create_asset(self, **params) -> bpy.types.Object:
        ...
        if self.has_step:
            ...
            parts.extend(steps)
            name.extend([f"steps_part" for i in range(len(steps))])
        if self.has_rail:
            rails = self.make_rails()
            parts.extend(rails)
            name.extend([f"rails_part" for i in range(len(rails))])
            # parts.extend(self.make_rails())
        if self.has_tread:
            treads = self.make_treads()
            parts.extend(treads)
            name.extend([f"treads_part" for i in range(len(treads))])
            # parts.extend(self.make_treads())
        if self.has_sides:
            inner_sides = self.make_inner_sides()
            parts.extend(inner_sides)
            name.extend([f"inner_sides_part" for i in range(len(inner_sides))])
            # parts.extend(self.make_inner_sides())
            outer_sides = self.make_outer_sides()
            parts.extend(outer_sides)
            name.extend([f"outer_sides_part" for i in range(len(outer_sides))])
            # parts.extend(self.make_outer_sides())
        handrails = self.make_handrails()
        parts.extend(handrails)
        name.extend([f"handrails_part" for i in range(len(handrails))])
        ...
        if self.has_vertical_post:
            ...
            parts.extend(posts)
            name.extend([f"vertical_posts_part" for i in range(len(posts))])
            # parts.extend()
        else:
            posts = self.make_posts(
                post_locs,
            ...
            parts.extend(posts)
            name.extend([f"posts_part" for i in range(len(posts))])
            # parts.extend(self.make_posts(post_locs, [self.post_width] * len(post_locs)))
        if self.has_bars:
            bars = self.make_bars(post_locs)
            parts.extend(bars)
            name.extend([f"bars_part" for i in range(len(bars))])
            # parts.extend(self.make_bars(post_locs))
        if self.has_glasses:
            glasses = self.make_glasses(post_locs)
            parts.extend(glasses)
            name.extend([f"glasses_part" for i in range(len(glasses))])
            # parts.extend(self.make_glasses(post_locs))
        # obj = join_objects(parts)
        obj = save_obj_parts_join_objects(
            parts, params.get("path", None), params.get("i", "unknown"), name=name, obj_name="Staircase", first=True
        )
        obj_name = "Staircase"
        if self.has_spiral:
            self.make_spiral(obj)
            obj_name = "SpiralStaircase"
        if self.has_column:
            ...
            save_objects_obj(
                ...
                params.get("path", None),
                params.get("i", "unknown"),
                name=["column"], obj_name=obj_name,
                first=False
            )
            obj = join_objects([obj, column])
        if self.mirror:
            mirror(obj)
        obj.rotation_euler[-1] = self.rot_z
        butil.apply_transform(obj)
    ...
    def make_guardrail(self, mesh, path="guardrail", i="unknown"):
        ...
        names = []
        parts.append(o)
        names.append("single_handrail")
        if self.has_bars:
            bars = self.make_bars(locs)
            parts.extend(bars)
            names.extend(["bar"] * len(bars))
        if self.has_glasses:
            glasses = self.make_glasses(locs)
            parts.extend(glasses)
            names.extend(["glass"] * len(glasses))
        parts.extend(
            self.make_posts(
                locs + minor_locs,
        ...
        butil.select_none()
        save_objects_obj(parts[: len(names)], path, i, names, first=False)
        obj = join_objects(parts)
        self.make_spiral(obj)
