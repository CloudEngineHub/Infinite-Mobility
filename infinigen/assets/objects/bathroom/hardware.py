...
class HardwareFactory(AssetFactory):
    ...
    def make_attachment(self, path=None, i="unknown", first=True):
        ...
        # save_obj_parts_add([base, rod], path, i, name=["base", "rod"], first=first)
    ...
    def create_asset(self, **params) -> bpy.types.Object:
        match self.hardware_type:
            ...
            case _:
                parts = self.make_attachment(params.get("path", None), params.get("i", "unknown"))
        ...
        if self.hardware_type == "bar":
            attachment_ = self.make_attachment(params.get("path", None), params.get("i", "unknown"), first=False)
            for att in attachment_:
                att.location[0] = self.bar_length
                butil.apply_transform(att, True)
            parts.extend(attachment_)
            name.extend(["base", "rod"])
        parts.append(extra)
        save_obj_parts_add(
            parts, params.get("path", None), params.get("i", "unknown"),
            name=name + [self.hardware_type], obj_name="Hardware", first=True
        )
        obj = join_objects(parts)
        join_objects_save_whole(obj, params.get("path", None), params.get("i", "unknown"),)
        obj.rotation_euler[-1] = np.pi / 2
