...
def nodegroup_create_anchors(nw: NodeWrangler, **kwargs):
    # Code generated using version 2.6.4 of the node_transpiler
    ...
    for i in range(kwargs.get("Profile N-gon", 0)):
        compare = nw.new_node(
...
def nodegroup_create_legs_and_strechers(nw: NodeWrangler, **kwargs):
    # Code generated using version 2.6.4 of the node_transpiler
    ...
    # Separate geometry here!
    # index = nw.new_node(Nodes.Index)
    leg_number = kwargs.get("Profile N-gon", 0)
    geometries = []
    ...
    for i in range(leg_number):
        ...
        realize_instances = nw.new_node(
            Nodes.RealizeInstances, input_kwargs={"Geometry": store_legs.outputs["Geometry"]}
        )
        geometries.append(realize_instances)

    if geometries:
        join_geometry_legs = nw.new_node(
            Nodes.JoinGeometry,
            input_kwargs={"Geometry": geometries},
        )
    else:
        join_geometry_legs = instance_on_points
    ...
    switch_1 = nw.new_node(
        Nodes.Switch,
        input_kwargs={1: group_input.outputs["Keep Legs"], 15: join_geometry_legs},
    )
    ...
    for i in range(leg_number):
        compare_1 = nw.new_node(
            Nodes.Compare, input_kwargs={0: named_attribute_1, 1: i}, attrs={"operation": "EQUAL"}
        )
        ...
        store_legs_1 = nw.new_node(
            Nodes.StoreNamedAttribute,
            input_kwargs={
                "Geometry": separate_geometry_1,
                "Name": "legs",
                "Value": i+leg_number+1,
            },
            attrs={"domain": "INSTANCE", "data_type": "INT"},
        )
        realize_instances_1 = nw.new_node(
            Nodes.RealizeInstances, input_kwargs={"Geometry": store_legs_1.outputs["Geometry"]}
        )
        geometries_1.append(realize_instances_1)

    if geometries_1:
        join_geometry_legs_1 = nw.new_node(
            Nodes.JoinGeometry,
            input_kwargs={"Geometry": geometries_1},
        )
    else:
        join_geometry_legs_1 = instance_on_points_1
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [switch_1.outputs[6], join_geometry_legs_1]},
    )
