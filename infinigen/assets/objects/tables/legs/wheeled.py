...
@node_utils.to_nodegroup(
    "nodegroup_chair_wheel_nocap", singleton=False, type="GeometryNodeTree"
)
def nodegroup_chair_wheel_nocap(nw: NodeWrangler):
    # Code generated using version 2.6.4 of the node_transpiler
    ...
    divide = nw.new_node(
        Nodes.Math,
        input_kwargs={0: group_input.outputs["Wheel Width"], 1: value},
        attrs={"operation": "DIVIDE"},
    )
    
    combine_xyz_7 = nw.new_node(Nodes.CombineXYZ, input_kwargs={"X": 1.000, "Y": divide, "Z": 1.000})
    ...
    join_geometry = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [store_wheel, store_wheel_cap, store_cylinder]},
    )
    ...
    join_geometry_1 = nw.new_node(
        Nodes.JoinGeometry,
        input_kwargs={"Geometry": [transform_4, store_subdivide]},
    )
...
@node_utils.to_nodegroup(
    "nodegroup_wheeled_leg_nocap", singleton=False, type="GeometryNodeTree"
)
def nodegroup_wheeled_leg_nocap(nw: NodeWrangler, **kwargs):
    # Code generated using version 2.6.4 of the node_transpiler
    ...
    createanchors = nw.new_node(
        nodegroup_create_anchors(**{"Profile N-gon": kwargs["Leg Pole Number"]}).name,
        input_kwargs={
            "Profile N-gon": group_input.outputs["Leg Number"],
            "Profile Width": value_1,
            "Profile Aspect Ratio": 1.0000,
    ...
    chair_wheel = nw.new_node(
        nodegroup_chair_wheel_nocap().name,
        input_kwargs={
            "Arc Sweep Angle": group_input.outputs["Arc Sweep Angle"],
            "Wheel Width": group_input.outputs["Wheel Width"],
            "Wheel Rotation": group_input.outputs["Wheel Rotation"],
    ...
    createlegsandstrechers = nw.new_node(
        nodegroup_create_legs_and_strechers(**{"Profile N-gon": kwargs["Leg Pole Number"]}).name,
        input_kwargs={
            "Anchors": createanchors,
            "Keep Legs": True,
            "Leg Instance": transform_geometry,
    ...
    realize_instance = nw.new_node(Nodes.RealizeInstances, [alignbottomtofloor.outputs["Geometry"]])

    store_legs_2 = nw.new_node(
            Nodes.StoreNamedAttribute,
            input_kwargs={
                "Geometry": transform_geometry_2,  # Geometry with UV map
                "Name": "legs",
                "Value": kwargs["Leg Pole Number"]+1,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "POINT", "data_type": "INT"},
        )
    
    store_legs_3 = nw.new_node(
            Nodes.StoreNamedAttribute,
            input_kwargs={
                "Geometry": transform_geometry_3,  # Geometry with UV map
                "Name": "legs",
                "Value": kwargs["Leg Pole Number"]+2,  # Assign Cube 1 an ID of 1
            },
            attrs={"domain": "POINT", "data_type": "INT"},
        )
