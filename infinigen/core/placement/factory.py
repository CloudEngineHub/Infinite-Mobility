...
class AssetFactory:
    ...
    def spawn_placeholder(self, i, loc, rot):
        ...
        with FixedSeed(int_hash((self.factory_seed, i))):
            obj = self.create_placeholder(
                i=i, loc=loc, rot=rot
            )  # butil.spawn_cube(size=2)
    ...
    def spawn_asset(
        ...
        **kwargs,
        ...
        view_layer_objects = dict(bpy.context.view_layer.objects)
        ...
        with (
            ...
            obj = self.create_asset(i=i, path=path, placeholder=placeholder, **params)

        obj.name = f"{repr(self)}.spawn_asset({i})"

        for name, object in view_layer_objects.items():
            if (name not in bpy.context.view_layer.objects) and (
                object not in bpy.context.view_layer.objects.values()
            ):
