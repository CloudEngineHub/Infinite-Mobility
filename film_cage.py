...
def generate_text_image(image):
    ...
    # 添加文字到图片上
    draw.text((10, 10), '洗衣机', font=font, fill=(0, 0, 0))
...
def main(id, catagory, path):
    ...
    if not os.path.exists(path_):
        generate_whole(f"{path}")
    mesh = trimesh.load(path_, force="mesh")
    ...
    while not viewer.closed:
        ...
        rgba_pil.save(f"pics__/screenshot{step}.png")
        step += 1
        print(step)
        if step == 300:
            break
        ...
        #print(step)
    os.system("ffmpeg -r 30 -i ./pics__/screenshot%d.png -vf palettegen ./pics__/palette.png")
    os.system(f"ffmpeg -r 30 -i ./pics__/screenshot%d.png -i ./pics__/palette.png -lavfi paletteuse ./pics__/gifs__/{catagory}_{id}.gif")
    os.system("rm ./pics__/palette.png")
    viewer.close()
    return True
...
if __name__ == "__main__":
    path = '/home/pjlab/下载/cage_urdf_0212/export/WashingMachine'
    starting_number = 0
